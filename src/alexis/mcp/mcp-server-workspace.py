...
# Flag to hide directories starting with a dot (e.g., .git, .agents), defaults to True
...
async def tool_list(path: str = "") -> List[str]:
    """Returns the immediate (non-recursive) contents of a directory.

    Lists the sub-directories and files directly inside `path` (relative to the
    workspace root; default is the workspace root). Directory names are suffixed
    with '/'. Does not descend into sub-directories.
    """
    try:
        # Resolve the target directory. An empty/root path lists the workspace
        # root; get_safe_path rejects the base directory itself, so only call it
        # for a real sub-path.
        if path and path.strip() not in (".", "./", "/"):
            myPath = get_safe_path(WORKSPACE_DIR, path)
        else:
        ...
        for entry in os.listdir(myPath):
            if os.path.isdir(os.path.join(myPath, entry)):
                # Hide dot-directories (e.g. .git, .agents) when configured.
                if HIDE_DOT_DIRS and entry.startswith("."):
                    continue
                dirs.append(entry + "/")
            else:
                files.append(entry)
        retV = sorted(dirs) + sorted(files)
